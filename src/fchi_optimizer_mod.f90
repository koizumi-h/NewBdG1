module fchi_optimizer_mod
  ...
  type, extends(circle_list), public :: fchi_optimizer
    ! c1:small loop, c2:large loop, c:all loop
    real(8), dimension(:), allocatable        ::  delta_chi, jump
    real(8)                            :: tol = 1.d-4, ratio = 0.2d0
    integer                            :: itermax = 10
  ...
  contains
    procedure :: fchi_optimizer_init
    !procedure :: draw_path    ! I made this subroutine just for debugging
    !procedure :: draw_int     ! for debugging
    procedure :: modify_init => fchi_optimizer_modify_init
    procedure :: load_cranking => fchi_optimizer_load_cranking
    procedure :: d_angval       => fchi_optimizer_d_angval
    procedure :: self_consistent => fchi_optimizer_self_consistent
    !procedure :: self_consistent2 => fchi_optimizer_self_consistent2
    procedure :: oneshot => fchi_optimizer_oneshot
    procedure :: get_Jp => fchi_optimizer_get_Jp
    !procedure :: get_Jp2 => fchi_optimizer_get_Jp2
    procedure :: get_dJdf => fchi_optimizer_get_dJdf
    !procedure :: get_dJdf2 => fchi_optimizer_get_dJdf2
    procedure :: get_n => fchi_optimizer_get_n
    procedure :: get_w => fchi_optimizer_get_w
    procedure :: set_lcp => fchi_optimizer_set_lcp
    procedure :: adjacent_path => fchi_optimizer_adjacent_path
    procedure :: calc_fdelta_chi => fchi_optimizer_calc_fdelta_chi
    procedure :: mix_delta_dchi => fchi_optimizer_mix_delta_dchi
    procedure :: set_jump => fchi_optimizer_set_jump
    procedure :: set_w1 => fchi_optimizer_set_w1
    procedure :: set_w2 => fchi_optimizer_set_w2
    procedure :: set_w => fchi_optimizer_set_w
    procedure :: set_param => fchi_optimizer_set_param
    procedure :: set_jex => fchi_optimizer_set_jex
    procedure :: set_prod_vv => fchi_optimizer_set_prod_vv
    procedure :: my_dgetrs
    procedure :: get_Jp_matrix => fchi_optimizer_get_Jp_matrix
  end type fchi_optimizer
contains

  subroutine fchi_optimizer_init(this, pshape, hole, ne, t, u, jd, lm, xi, chi, wf, eg, ne_u, ne_d)
    class(fchi_optimizer), intent(inout)                :: this
    ...
    !stop
  end subroutine fchi_optimizer_init


  function fchi_optimizer_d_angval(this, val) result(r)
    class(fchi_optimizer), intent(in)        :: this
  ...
  end function fchi_optimizer_d_angval



  subroutine fchi_optimizer_modify_init(this, hole, xi, chi, wf)
    !------------
    ! this subroutine modifies fchi_optimizer_init but do not executes
    ...
    call this%set_jump()
    call this%set_prod_vv()
  end subroutine fchi_optimizer_modify_init


  subroutine fchi_optimizer_load_cranking(this)
    class(fchi_optimizer), intent(inout)            :: this
    integer                                         :: access
    ...
    read(31) this%EV
    close(31)
  end subroutine fchi_optimizer_load_cranking

  subroutine fchi_optimizer_self_consistent(this, optimized)
    ! This subroutine is main program of fchi_optimizer,
  ...
  end subroutine fchi_optimizer_self_consistent

  !subroutine fchi_optimizer_self_consistent2(this,EV, optimized)
  !  ! This subroutine is main program of fchi_optimizer,
  !  ! and calculate d_chi = chi_k - chi_j by the self-consistent culculation
  !  ! of current conservation.
  !  ! The chi is consist of the mult-valued function chi_0 and the single valued function fchi.
  ...
  !    optimized = .false.
  !  end if
  !end subroutine fchi_optimizer_self_consistent2


  !subroutine fchi_optimizer_self_consistent_oneshot(this, dchi_0, df_result)
  subroutine fchi_optimizer_oneshot(this)
    ! This subroutine is main program of fchi_optimizer,
    ! and calculate d_chi = chi_k - chi_j by the self-consistent culculation
    ! of current conservation.
    ! The chi is consist of the mult-valued function chi_0 and the single valued function fchi.
    ...
    dchi = dchi + this%calc_fdelta_chi(Jp, dJdf)
    !dchi = dchi - this%calc_fdelta_chi(Jp, dJdf)*this%ratio
    allocate(this%delta_chi(this%np), source=dchi)
  end subroutine fchi_optimizer_oneshot


  function fchi_optimizer_get_Jp(this, dchi) result(r)
    class(fchi_optimizer), intent(in),target         :: this
...
! path[3] rashba (upper-right direction path around hole)
 ...
 ! path[3,4,5,6] p_rh and p_2nd   * p_rh are conteined in p_2nd
  ...
  ! path[7]  inter-layer path
    ...
    enddo
  end function fchi_optimizer_get_Jp

 ! function fchi_optimizer_get_Jp2(this, dchi, EV) result(r)
 !   class(fchi_optimizer), intent(in),target         :: this
 !   real(8), dimension(this%np), intent(in)     :: dchi
 !   real(8), dimension(this%np)                :: r
 ...
 !   !enddo
 ! end function fchi_optimizer_get_Jp2

  function fchi_optimizer_get_dJdf(this, dchi) result(r)
    class(fchi_optimizer), intent(in), target  :: this
    real(8), dimension(this%np), intent(in)     :: dchi
    real(8), dimension(this%np)                :: r
    !real(8), dimension(4*this%n_acc)               :: r_temp
...
! path[7]  inter-layer path
    ...
    !do k = this%np_1st + this%np_rh + 1, this%np !! this is ignored p_2nd : when p_2nd = 0
      ...
      r(k) = r_temp1
    enddo
  end function fchi_optimizer_get_dJdf

  !function fchi_optimizer_get_dJdf2(this, dchi, EV) result(r)
  !  class(fchi_optimizer), intent(in), target  :: this
  !  real(8), dimension(this%np), intent(in)     :: dchi
  !  real(8), dimension(this%np)                :: r
  !  real(8), dimension(this%ne)                :: r_temp
...
! !path[3,4]
  ...
  !    r(i) = val
  !  end do
  !end function fchi_optimizer_get_dJdf2

  subroutine fchi_optimizer_set_lcp(this)
    class(fchi_optimizer), intent(inout) :: this
    integer                              :: i, j, k, l
    ...
    end subroutine fchi_optimizer_set_lcp

  function fchi_optimizer_get_n(this, dJdf) result(r)
    use ieee_arithmetic
    class(fchi_optimizer), intent(in)       :: this
    ...
    do i = 1, this%nc
      ...
      enddo
    enddo
  end function fchi_optimizer_get_n

  function fchi_optimizer_get_w(this, Jp) result(r)
    class(fchi_optimizer), intent(in)       :: this
    ...
    !enddo
  end function fchi_optimizer_get_w

  subroutine fchi_optimizer_adjacent_path(this)
    class(fchi_optimizer), intent(inout)        :: this
    integer i, j, k
    ...
    do i = 1, this%n_acc
      ...
      deallocate (p)
    enddo
  end subroutine fchi_optimizer_adjacent_path

  function fchi_optimizer_calc_fdelta_chi(this, Jp, dJdf) result(r)
    class(fchi_optimizer), intent(inout)       :: this
    real(8), dimension(this%np), intent(in)  :: Jp
    real(8), dimension(this%np), intent(in)  :: dJdf
    real(8), dimension(this%np)              :: r
    ...
    !write(*, *) 'info', info
  end function fchi_optimizer_calc_fdelta_chi

  function fchi_optimizer_mix_delta_dchi(this, odf, df, ratio) &
  & result(r)
    class(fchi_optimizer), intent(in)      :: this
  ...
  end function fchi_optimizer_mix_delta_dchi

  subroutine fchi_optimizer_set_jump(this)
    class(fchi_optimizer), intent(inout) :: this
    !real(8), dimension(this%np)          :: dchi_0
    real(8)                              :: jmp
    integer                              :: i, k, j
  ...
  end subroutine fchi_optimizer_set_jex
  
  subroutine fchi_optimizer_set_param(this, xi, chi, wf, itermax, ratio, tol, pflag)
    class(fchi_optimizer), intent(inout) :: this
    ...
    real(8), intent(in), optional :: ratio, tol
  ...
  end subroutine fchi_optimizer_set_param

  subroutine fchi_optimizer_set_prod_vv(this)
    class(fchi_optimizer), intent(inout) :: this
    integer k, a, i, f
    this%vv_uu = 0d0
    this%vv_dd = 0d0
...
!    end do
  ...
  end subroutine fchi_optimizer_set_prod_vv
  ...
  subroutine fchi_optimizer_get_Jp_matrix(this, jp_mat)
    class(fchi_optimizer), intent(in) :: this
    ...
    !end do
    !print *,tmp_jp
  end subroutine fchi_optimizer_get_Jp_matrix
  ...
  ! constructor
  function new_fchi_optimizer(pshape, hole, ne, t, u, jd, lm, xi, chi, wf, eg, ne_u, ne_d) result(r)
    integer, dimension(:, :), intent(in)                   :: pshape
    integer, dimension(:, :), intent(in)                :: hole
    real(8), dimension(:), intent(in)                   :: ne, t
    real(8), intent(in)                                 :: u, jd, lm
    ...
    complex(8), dimension(:, :), intent(in)      :: wf
    real(8), dimension(:), intent(in)                   :: eg, ne_u, ne_d
    type(fchi_optimizer)                                :: r
    !type(cuo2_plane), dimension(:), allocatable         :: layers
    !character(1), optional                   :: rs
    !if (present(rs)) then
      !call r%fchi_optimizer_init(pshape, hole, ne, t, u, jd, lm, xi, chi, wf )
      call r%fchi_optimizer_init(pshape, hole, ne, t, u, jd, lm, xi, chi, wf, eg, ne_u, ne_d)
    !else
    !  call r%fchi_optimizer_init(pshape, hole, t, u, jd, lm, xi, chi, wf)
    !endif
  end function new_fchi_optimizer
end module fchi_optimizer_mod
