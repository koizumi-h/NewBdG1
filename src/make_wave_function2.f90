PROGRAM make_wave_function2
  ...
  if (lm /= 0.d0) then
    ...
    !open(121, file = "xi1_eg2.dat")
    !do i = 1, 100
    !  p(1) = 2d0*pi*(i-1d0)/100d0
    !  write(121, *)  p(1) , cgr%get_func( p ), cgr%get_dfunc( p )
    !end do
    !close(121)
    ...
    !  ALLOCATE(an_xi)
    !  ! xi1_optimize by rashba_spin_orbit_coupling
    !  an_xi = new_angular_variable_3D (pshape,hole_xi)
    !  allocate (cgr)
    !  cgr = new_conjugate_gradient_rsoc(n, n_acc, lm, an_xi, wf, xi, chi)
    !  p(1) = 0.6d0*pi ! initial xi1
    ...
    !    call fopt%modify_init(hole_chi, xi, chi, wf)
    ...
    !  endif
    !  !call draw_phase("cpxiv.gp", unit_num, pshape, xi, hole_xi(:, 1))
    !  !write(*, *) 'optimized xi1', xi(1)
    !  p(1) = modulo(xi(1), 2.d0*pi)
    !  write(*, *) 'optimized xi1', p(1)
    !  xi = an_xi%rebuilt_angval_xi(xi, p(1))
    ...
    ! do i = 1, 100
    !   xi = an_xi%rebuilt_angval_xi(xi, 2d0*pi*(i-1d0)/100d0 )
    !   write(121, *) xi(1) , total_energy_sb_2lay_xichi_T0(fopt, wf, xi, chi)
    ! end do
    ! xi = an_xi%rebuilt_angval_xi(xi, p(1))
    ! close(121)
    ! !DEALLOCATE(an_xi)
  ...
  call draw_spin("fchi_spin.dat", 114, pshape, Sx, Sy, Sz)
  call draw_spin_normalize("fchi_spin_n.dat", 115, pshape, Sx, Sy, Sz)
  print *, "saved to ""fchi_spin.dat"", ""fchi_spin_n.dat"" "
 ...
 ! for stm, arpes, etc.
  write(120) eg
  write(121) wf1 ! chiの因子を含む波動関数(一価)
  write(122) wf2 ! chiの因子を含まない波動関数(xiは含む,多価)
...
END PROGRAM make_wave_function2